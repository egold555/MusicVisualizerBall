...
var debug = false;
var canvas = document.getElementById("debugMenu");
var ctx = canvas.getContext("2d");
window.onload = function(){
  var webgl = new Webgl();
  var audio = new Audio(webgl);
  $("#debugMenu").children().hide(); 
  $("#debugMenu").hide(); 
...
var canvasFps = 0;
var canvasFreqAvg = 0;
var canvasSpectrum = 0;
function drawDebugMenu(){
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.font = "16px Arial";
  ctx.fillStyle = "white";
  ctx.fillText("FPS: " + canvasFps,30,30);
  ctx.fillText("FreqAvg: " + canvasFreqAvg,30,45);
  ctx.fillText("Spectrum: " + canvasSpectrum,30,60);
}
  ...
  init(){
    ...
    this.fileReader.onload = function(){
      _this.audioContext.decodeAudioData(_this.fileReader.result, function(buffer){
        ...
        var shouldLoop = document.getElementById("loopaudio").checked;
        _this.source.loop = shouldLoop;
